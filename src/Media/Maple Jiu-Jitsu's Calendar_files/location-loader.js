(function (window) {
	const moduleName = 'lucy-location';
	const baseUrl = 'https://assets.setmore.com/location';
	...
	const getEnvironment = () => {
		let env =
			FrontOfficeService?.App?.getAppEnvironment() === 'live'
				? 'live'
				: 'staging';
		if (isPreAlpha()) {
	...
	const injectLocationScript = (src) => {
		return new Promise((resolve, reject) => {
	...
	const injectAssetsInDocument = async (paths) => {
		if (paths) {
			...
			await injectLocationScript(paths['location.js.br']);
			return true;
		}
	...
	const init = async (param) => {
		try {
			...
			if (status && window[moduleName]?.initLocation) {
				return window[moduleName].initLocation(param);
			} else {
				throw new Error('LC:initLocation Method not found');
			}
		} catch (error) {
			console.error(
				'LC:Something went wrong while initializing location:',
				error
			);
